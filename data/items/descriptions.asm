ItemDescriptions:
; entries correspond to item ids
	dw MasterBallDesc
	dw UltraBallDesc
	dw BrightpowderDesc
	dw GreatBallDesc
	dw PokeBallDesc
	dw TeruSama1Desc
	dw BicycleDesc
	dw MoonStoneDesc
	dw AntidoteDesc
	dw BurnHealDesc
	dw IceHealDesc
	dw AwakeningDesc
	dw ParlyzHealDesc
	dw FullRestoreDesc
	dw MaxPotionDesc
	dw HyperPotionDesc
	dw SuperPotionDesc
	dw PotionDesc
	dw EscapeRopeDesc
	dw RepelDesc
	dw MaxElixerDesc
	dw FireStoneDesc
	dw ThunderStoneDesc
	dw WaterStoneDesc
	dw CovenantOrbDesc
	dw HPUpDesc
	dw ProteinDesc
	dw IronDesc
	dw CarbosDesc
	dw LuckyPunchDesc
	dw CalciumDesc
	dw RareCandyDesc
	dw XAccuracyDesc
	dw LeafStoneDesc
	dw MetalPowderDesc
	dw NuggetDesc
	dw PokeDollDesc
	dw FullHealDesc
	dw ReviveDesc
	dw MaxReviveDesc
	dw GuardSpecDesc
	dw SuperRepelDesc
	dw MaxRepelDesc
	dw DireHitDesc
	dw PoisonStoneDesc
	dw FreshWaterDesc
	dw SodaPopDesc
	dw LemonadeDesc
	dw XAttackDesc
	dw OldAmberDesc
	dw XDefendDesc
	dw XSpeedDesc
	dw XSpecialDesc
	dw CoinCaseDesc
	dw ItemfinderDesc
	dw RoomPassDesc
	dw ExpShareDesc
	dw OldRodDesc
	dw GoodRodDesc
	dw SilverLeafDesc
	dw SuperRodDesc
	dw PPUpDesc
	dw EtherDesc
	dw MaxEtherDesc
	dw ElixerDesc
	dw RedScaleDesc
	dw SecretPotionDesc
	dw SSTicketDesc
	dw MysteryEggDesc
	dw ClearBellDesc
	dw SilverWingDesc
	dw MoomooMilkDesc
	dw QuickClawDesc
	dw PsnCureBerryDesc
	dw GoldLeafDesc
	dw SoftSandDesc
	dw SharpBeakDesc
	dw PrzCureBerryDesc
	dw BurntBerryDesc
	dw IceBerryDesc
	dw PoisonBarbDesc
	dw KingsRockDesc
	dw BitterBerryDesc
	dw MintBerryDesc
	dw RedApricornDesc
	dw TinyMushroomDesc
	dw BigMushroomDesc
	dw SilverPowderDesc
	dw BluApricornDesc
	dw SkateboardDesc
	dw AmuletCoinDesc
	dw YlwApricornDesc
	dw GrnApricornDesc
	dw CleanseTagDesc
	dw MysticWaterDesc
	dw TwistedSpoonDesc
	dw WhtApricornDesc
	dw BlackbeltDesc
	dw BlkApricornDesc
	dw LongVineDesc
	dw PnkApricornDesc
	dw BlackGlassesDesc
	dw SlowpokeTailDesc
	dw PinkBowDesc
	dw StickDesc
	dw SmokeBallDesc
	dw NeverMeltIceDesc
	dw MagnetDesc
	dw MiracleBerryDesc
	dw PearlDesc
	dw BigPearlDesc
	dw EverStoneDesc
	dw SpellTagDesc
	dw RageCandyBarDesc
	dw LongTongueDesc
	dw TeruSama10Desc
	dw MiracleSeedDesc
	dw ThickClubDesc
	dw FocusBandDesc
	dw AppleDesc
	dw EnergyPowderDesc
	dw EnergyRootDesc
	dw HealPowderDesc
	dw RevivalHerbDesc
	dw HardStoneDesc
	dw LuckyEggDesc
	dw CardKeyDesc
	dw MachinePartDesc
	dw TeruSama12Desc
	dw LostItemDesc
	dw StardustDesc
	dw StarPieceDesc
	dw BasementKeyDesc
	dw PassDesc
	dw SnakeskinDesc
	dw PrettyTailDesc
	dw EarthenClayDesc
	dw CharcoalDesc
	dw BerryJuiceDesc
	dw ScopeLensDesc
	dw StimulsOrbDesc
	dw ConfuseClawDesc
	dw MetalCoatDesc
	dw DragonFangDesc
	dw FocusOrbDesc
	dw LeftoversDesc
	dw CalmSeedDesc
	dw WaterTailDesc
	dw FireTailDesc
	dw MysteryBerryDesc
	dw DragonScaleDesc
	dw BerserkGeneDesc
	dw ThunderTailDesc
	dw WisdomOrbDesc
	dw DetectOrbDesc
	dw SacredAshDesc
	dw HeavyBallDesc
	dw FlowerMailDesc
	dw LevelBallDesc
	dw LureBallDesc
	dw FastBallDesc
	dw SharpHornsDesc
	dw LightBallDesc
	dw FriendBallDesc
	dw MoonBallDesc
	dw LoveBallDesc
	dw NormalBoxDesc
	dw GorgeousBoxDesc
	dw SunStoneDesc
	dw PolkadotBowDesc
	dw PowerWingsDesc
	dw UpGradeDesc
	dw BerryDesc
	dw GoldBerryDesc
	dw SquirtBottleDesc
	dw EnergyWallDesc
	dw ParkBallDesc
	dw RainbowWingDesc
	dw SteelShellDesc
	dw BrickPieceDesc
	dw SurfMailDesc
	dw LiteBlueMailDesc
	dw PortraitMailDesc
	dw LovelyMailDesc
	dw EonMailDesc
	dw MorphMailDesc
	dw BlueSkyMailDesc
	dw MusicMailDesc
	dw MirageMailDesc
	dw TeruSama29Desc
	dw TeruSama30Desc
	dw TeruSama30Desc
	dw TeruSama30Desc
	dw TeruSama30Desc
	dw TeruSama30Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama31Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama32Desc
	dw TeruSama33Desc
	dw TeruSama34Desc
	dw TeruSama35Desc
	dw TeruSama36Desc
	dw TeruSama37Desc
	dw TeruSama37Desc

MasterBallDesc:
	db   "Der beste BALL."
	next "Erfolg garantiert!@"

UltraBallDesc:
	db   "Ein BALL mit hoher"
	next "Erfolgsquote.@"

BrightpowderDesc:
	db   "Senkt Genauigkeit."
	next "(TRAGEN)@"

GreatBallDesc:
	db   "Ein BALL mit guter"
	next "Erfolgsquote.@"

PokeBallDesc:
	db   "Damit fängst du"
	next "#MON.@"

TeruSama1Desc:
	db   "?@"

BicycleDesc:
	db   "Ein Klapprad, um"
	next "schneller zu sein.@"

MoonStoneDesc:
	db   "Entwickelt"
	next "manche #MON.@"

AntidoteDesc:
	db   "Heilt"
	next "Vergiftungen.@"

BurnHealDesc:
	db   "Heilt"
	next "Verbrennungen.@"

IceHealDesc:
	db   "Taut #MON"
	next "wieder auf.@"

AwakeningDesc:
	db   "Weckt schlafende"
	next "#MON.@"

ParlyzHealDesc:
	db   "Heilt "
	next "Paralyse.@"

FullRestoreDesc:
	db   "Stellt KP und"
	next "Status wieder her.@"

MaxPotionDesc:
	db   "Füllt KP"
	next "vollständig auf.@"

HyperPotionDesc:
	db   "Füllt KP um"
	next "200 auf.@"

SuperPotionDesc:
	db   "Füllt KP um"
	next "50 auf.@"

PotionDesc:
	db   "Füllt KP um"
	next "20 auf.@"

EscapeRopeDesc:
	db   "Ermöglicht Flucht"
	next "aus Höhlen, etc.@"

RepelDesc:
	db   "Hält schwache"
	next "#MON ab.@"

MaxElixerDesc:
	db   "Füllt alle AP"
	next "auf.@"

FireStoneDesc:
	db   "Entwickelt manche"
	next "#MON.@"

ThunderStoneDesc:
	db   "Entwickelt manche"
	next "#MON.@"

WaterStoneDesc:
	db   "Entwickelt manche"
	next "#MON.@"
	
CovenantOrbDesc:
	db   "Entwickelt manche"
	next "#MON.@"

TeruSama2Desc:
	db   "?@"

HPUpDesc:
	db   "Erhöht KP"
	next "eines #MON.@"

ProteinDesc:
	db   "Erhöht den"
	next "ANGR-Wert.@"

IronDesc:
	db   "Erhöht den"
	next "VERT-Wert.@"

CarbosDesc:
	db   "Erhöht den"
	next "INIT-Wert.@"

LuckyPunchDesc:
	db   "Erhöht CHANEIRAs"
	next "Volltrefferquote.@"

CalciumDesc:
	db   "Erhöht den"
	next "SPEZ-Wert.@"

RareCandyDesc:
	db   "Hebt Level um"
	next "eins an.@"

XAccuracyDesc:
	db   "Hebt Genauigkeit"
	next "für 1 Kampf.@"

LeafStoneDesc:
	db   "Entwickelt manche"
	next "#MON.@"

MetalPowderDesc:
	db   "Hebt VERT-Wert von"
	next "DITTO. (TRAGEN)@"

NuggetDesc:
	db   "Aus purem Gold."
	next "Wert hoch.@"

PokeDollDesc:
	db   "Flucht vor wilden"
	next "#MON.@"

FullHealDesc:
	db   "Beseitigt alle"
	next "Statusprobleme.@"

ReviveDesc:
	db   "Belebt #MON"
	next "plus halbe KP.@"

MaxReviveDesc:
	db   "Belebt #MON"
	next "plus volle KP.@"

GuardSpecDesc:
	db   "Keine Statusände-"
	next "rung für 1 Kampf.@"

SuperRepelDesc:
	db   "Starker Schutz vor"
	next "#MON.@"

MaxRepelDesc:
	db   "Bester Schutz vor"
	next "#MON.@"

DireHitDesc:
	db   "Hebt Volltreffer-"
	next "quote für 1 Kampf.@"


PoisonStoneDesc:
	db   "Entwickelt manche"
	next "#MON.@"

FreshWaterDesc:
	db   "Füllt KP um"
	next "50 auf.@"

SodaPopDesc:
	db   "Füllt KP um"
	next "60 auf.@"

LemonadeDesc:
	db   "Füllt KP um"
	next "80 auf.@"

XAttackDesc:
	db   "Hebt ANGR-Wert"
	next "für 1 Kampf.@"

OldAmberDesc:
	db   "Ein Rätselhaftes"
	next "#MON-Fossil.@"

XDefendDesc:
	db   "Hebt VERT-Wert"
	next "für 1 Kampf.@"

XSpeedDesc:
	db   "Hebt INIT-Wert"
	next "für 1 Kampf.@"

XSpecialDesc:
	db   "Hebt SPEZ. ANG"
	next "für 1 Kampf.@"

CoinCaseDesc:
	db   "Kann bis zu 9.999"
	next "Münzen aufnehmen.@"

ItemfinderDesc:
	db   "Scannt die Umge-"
	next "bung nach Items.@"

RoomPassDesc:
	db   "Eine Zimmerkarte"
	next "des RYUKYU HOTELs.@"

ExpShareDesc:
	db   "Teilt Kampf-EP."
	next "(TRAGEN)@"

OldRodDesc:
	db   "Damit kannst du"
	next "#MON angeln.@"

GoodRodDesc:
	db   "Eine gute ANGEL"
	next "zum Fischen.@"

SilverLeafDesc:
	db   "Ein seltsames"
	next "silbernes Blatt.@"

SuperRodDesc:
	db   "Die beste ANGEL"
	next "zum Fischen.@"

PPUpDesc:
	db   "Hebt die max."
	next "Anzahl der AP.@"

EtherDesc:
	db   "Füllt AP um"
	next "10 auf.@"

MaxEtherDesc:
	db   "Füllt AP"
	next "vollständig auf.@"

ElixerDesc:
	db   "Füllt alle AP"
	next "um 10 auf.@"

RedScaleDesc:
	db   "Haut vom roten"
	next "GARADOS.@"

SecretPotionDesc:
	db   "Heilt jedes #-"
	next "MON vollständig.@"

SSTicketDesc:
	db   "Ein Ticket für die"
	next "M.S. AQUA.@"

MysteryEggDesc:
	db   "Ein EI von"
	next "MR. #MON.@"

ClearBellDesc:
	db   "Erzeugt ein"
	next "angenehmes Läuten.@"

SilverWingDesc:
	db   "Eine seltsame"
	next "silberne Feder.@"

MoomooMilkDesc:
	db   "Füllt KP um"
	next "100 auf.@"

QuickClawDesc:
	db   "Erhöht Erstschlag-"
	next "quote. (TRAGEN)@"

PsnCureBerryDesc:
	db   "Selbstheilung bei"
	next "Gift. (TRAGEN)@"

GoldLeafDesc:
	db   "Ein seltsames"
	next "goldenes Blatt.@"

SoftSandDesc:
	db   "Verstärkt Boden-"
	next "Attacken. (TRAGEN)@"

SharpBeakDesc:
	db   "Verstärkt Flug-"
	next "Attacken. (TRAGEN)@"

PrzCureBerryDesc:
	db   "Selbstheilung bei"
	next "Paralyse. (TRAGEN)@"

BurntBerryDesc:
	db   "Selbstheilung bei"
	next "Frost. (TRAGEN)@"

IceBerryDesc:
	db   "Selbstheilung bei"
	next "Brand. (TRAGEN)@"

PoisonBarbDesc:
	db   "Verstärkt Gift-"
	next "Attacken. (TRAGEN)@"

KingsRockDesc:
	db   "Feind flieht"
	next "evtl. (TRAGEN)@"

BitterBerryDesc:
	db   "Selbstheilung bei"
	next "Verwirr. (TRAGEN)@"

MintBerryDesc:
	db   "Auto-Aufwecker bei"
	next "Schlaf. (TRAGEN)@"

RedApricornDesc:
	db   "Eine rote"
	next "APRIKOKO.@"

TinyMushroomDesc:
	db   "Ein gemeiner Pilz."
	next "Wert gering.@"

BigMushroomDesc:
	db   "Ein seltener Pilz."
	next "Wert hoch.@"

SilverPowderDesc:
	db   "Verstärkt Käfer-"
	next "Attacken. (TRAGEN)@"

BluApricornDesc:
	db   "Eine blaue"
	next "APRIKOKO.@"

SkateboardDesc:
	db   "Ein Skateboard um"
	next "schneller zu sein.@"

AmuletCoinDesc:
	db   "Doppeltes Ein-"
	next "kommen. (TRAGEN)@"

YlwApricornDesc:
	db   "Eine gelbe"
	next "APRIKOKO.@"

GrnApricornDesc:
	db   "Eine grüne"
	next "APRIKOKO.@"

CleanseTagDesc:
	db   "Schutz vor wilden"
	next "#MON. (TRAGEN)@"

MysticWaterDesc:
	db   "Verstärkt Wasser-"
	next "Attacken. (TRAGEN)@"

TwistedSpoonDesc:
	db   "Verstärkt Psycho-"
	next "Attacken. (TRAGEN)@"

WhtApricornDesc:
	db   "Eine weiße"
	next "APRIKOKO.@"

BlackbeltDesc:
	db   "Verstärkt Kampf-"
	next "Attacken. (TRAGEN)@"

BlkApricornDesc:
	db   "Eine schwarze"
	next "APRIKOKO.@"

LongVineDesc:
	db   "Erhöht die"
	next "Fangrate. (TRAGEN)@"

PnkApricornDesc:
	db   "Eine pinke"
	next "APRIKOKO.@"

BlackGlassesDesc:
	db   "Verstärkt Unlicht-"
	next "Attacken. (TRAGEN)@"

SlowpokeTailDesc:
	db   "Sehr lecker."
	next "Wert hoch.@"

PinkBowDesc:
	db   "Verstärkt Normal-"
	next "Attacken. (TRAGEN)@"

StickDesc:
	db   "Gemeiner Lauch."
	next "Wert gering.@"

SmokeBallDesc:
	db   "Flucht vor wilden"
	next "#MON. (TRAGEN)@"

NeverMeltIceDesc:
	db   "Verstärkt Eis-"
	next "Attacken. (TRAGEN)@"

MagnetDesc:
	db   "Verstärkt Elektro-"
	next "Attacken. (TRAGEN)@"

MiracleBerryDesc:
	db   "Setzt Status in"
	next "Urzustand.(TRAGEN)@"

PearlDesc:
	db   "Eine Perle."
	next "Wert niedrig.@"

BigPearlDesc:
	db   "Eine schöne Perle."
	next "Wert hoch.@"

EverStoneDesc:
	db   "Entwicklungsstopp."
	next "(TRAGEN)@"

SpellTagDesc:
	db   "Verstärkt Geist-"
	next "Attacken. (TRAGEN)@"

RageCandyBarDesc:
	db   "Füllt KP um"
	next "20 auf.@"

LongTongueDesc:
	db   "Erhöht die"
	next "Fangrate. (TRAGEN)@"

TeruSama10Desc:
	db   "?@"

MiracleSeedDesc:
	db   "Verstärkt Pfl.-"
	next "Attacken. (TRAGEN)@"

ThickClubDesc:
	db   "Ein Knochen."
	next "Wert niedrig.@"

FocusBandDesc:
	db   "Könnte K.O. ver-"
	next "hindern. (TRAGEN)@"


AppleDesc:
	db   "Selbstheilung um"
	next "50 KP (TRAGEN)@"

EnergyPowderDesc:
	db   "Füllt KP um"
	next "50 auf. Bitter.@"

EnergyRootDesc:
	db   "Füllt KP um"
	next "200 auf. Bitter.@"

HealPowderDesc:
	db   "Heilt alle Status-"
	next "probleme. Bitter.@"

RevivalHerbDesc:
	db   "Belebt besiegte"
	next "#MON wieder.@"

HardStoneDesc:
	db   "Verstärkt Gestein-"
	next "Attacken. (TRAGEN)@"

LuckyEggDesc:
	db   "Erhält Extra-EP."
	next "(TRAGEN)@"

CardKeyDesc:
	db   "Eine Benzinleitung"
	next "für die M.S.AQUA.@"

MachinePartDesc:
	db   "Mystische Kugel"
	next "aus der Höhle.@"      ;evtl. anpassen?

TeruSama12Desc:
	db   "?@"

LostItemDesc:
	db   "#PUPPE der"
	next "NACHAHMERIN.@"

StardustDesc:
	db   "Schöner roter"
	next "Sand. Wert hoch.@"

StarPieceDesc:
	db   "Roter Edelstein."
	next "Wert sehr hoch.@"

BasementKeyDesc:
	db   "Öffnet Türen.@"

PassDesc:
	db   "Ein Ticket für den"
	next "KANTO-KAMPFKLUB.@"

SnakeskinDesc:
	db   "Verhindert "
	next "Vergif. (TRAGEN)@"

PrettyTailDesc:
	db   "Verhindert "
	next "Vergif. (TRAGEN)@"

EarthenClayDesc:
	db   "Verhindert"
	next "Paralyse. (TRAGEN)@"

CharcoalDesc:
	db   "Verstärkt Feuer-"
	next "Attacken. (TRAGEN)@"

BerryJuiceDesc:
	db   "Füllt KP um"
	next "20 auf.@"

ScopeLensDesc:
	db   "Hebt Volltreffer-"
	next "quote. (TRAGEN)@"

StimulsOrbDesc:
	db   "Verhindert"
	next "Schlafen. (TRAGEN)@"

ConfuseClawDesc:
	db   "Verhindert"
	next "Verwirr. (TRAGEN)@"

MetalCoatDesc:
	db   "Verstärkt Stahl-"
	next "Attacken. (TRAGEN)@"

DragonFangDesc:
	db   "Verstärkt Drachen-"
	next "Attacken. (TRAGEN)@"

FocusOrbDesc:
	db   "Erhöht GENAUIGKEIT"
	next "einmalig. (TRAGEN)@"

LeftoversDesc:
	db   "Füllt KP im Kampf"
	next "auf. (TRAGEN)@"

CalmSeedDesc:
	db   "Verhindert"
	next "Verwirr. (TRAGEN)@"

WaterTailDesc:
	db   "Verhindert"
	next "Brand. (TRAGEN)@"

FireTailDesc:
	db   "Verhindert"
	next "Frost. (TRAGEN)@"

MysteryBerryDesc:
	db   "Auto-Auffüllung"
	next "der AP. (TRAGEN)@"

DragonScaleDesc:
	db   "Ein seltenes"
	next "Drachen-Item.@"

BerserkGeneDesc:
	db   "Hebt ANGR-Wert und"
	next "bewirkt Verwirr.@"

ThunderTailDesc:
	db   "Verhindert"
	next "Paralyse. (TRAGEN)@"

WisdomOrbDesc:
	db   "Erhöht SPEZ. ANGR"
	next "einmalig. (TRAGEN)@"

DetectOrbDesc:
	db   "Erhöht FLUCHTWERT"
	next "einmalig. (TRAGEN)@"

SacredAshDesc:
	db   "Belebt alle be-"
	next "siegten #MON.@"

HeavyBallDesc:
	db   "Damit fängst du"
	next "schwere #MON.@"

FlowerMailDesc:
	db   "Umschlag mit Blu-"
	next "mendruck.(TRAGEN)@"

LevelBallDesc:
	db   "Ein BALL für"
	next "schwache #MON.@"

LureBallDesc:
	db   "Ein BALL für #-"
	next "MON an der Angel.@"

FastBallDesc:
	db   "Ein BALL für"
	next "schnelle #MON.@"

SharpHornsDesc:
	db   "Erhöht ANGRIFF"
	next "einmalig. (TRAGEN)@"

LightBallDesc:
	db   "Bizarrer Elektro-"
	next "Ball. (TRAGEN)@"

FriendBallDesc:
	db   "Fördert Sympathie"
	next "bei wilden PKMN.@"

MoonBallDesc:
	db   "Fängt MONDST.-Ent-"
	next "wickler eher ein.@"

LoveBallDesc:
	db   "Fängt anderes Ge-"
	next "schlecht leichter.@"

NormalBoxDesc:
	db   "Öffne sie und"
	next "schau hinein.@"

GorgeousBoxDesc:
	db   "Öffne sie und"
	next "schau hinein.@"

SunStoneDesc:
	db   "Entwickelt manche"
	next "#MON.@"

PolkadotBowDesc:
	db   "Verstärkt Normal-"
	next "Attacken. (TRAGEN)@"

PowerWingsDesc:
	db   "Erhöht INITIATIVE"
	next "einmalig. (TRAGEN)@"

UpGradeDesc:
	db   "Eine mysteriöse"
	next "Box der SILPH CO.@"

BerryDesc:
	db   "Selbstheilung um"
	next "10 KP. (TRAGEN)@"

GoldBerryDesc:
	db   "Selbstheilung um"
	next "30 KP. (TRAGEN)@"

SquirtBottleDesc:
	db   "Eine Kanne, um"
	next "Bäume zu wässern.@"

EnergyWallDesc:
	db   "Erhöht SPEZ. VERT"
	next "einmalig. (TRAGEN)@"

ParkBallDesc:
	db   "BALL für das"
	next "Käferturnier.@"

RainbowWingDesc:
	db   "Mystische Regen-"
	next "bogen-Feder. @"

SteelShellDesc:
	db   "Erhöht VERT"
	next "einmalig. (TRAGEN)@"

BrickPieceDesc:
	db   "Ein seltenes Stück"
	next "eines Ziegels.@"

SurfMailDesc:
	db   "Umschlag mit"
	next "LAPRAS. (TRAGEN)@"

LiteBlueMailDesc:
	db   "Umschlag mit"
	next "DRATINI. (TRAGEN)@"

PortraitMailDesc:
	db   "Umschlag mit"
	next "Besitzerportrait.@"

LovelyMailDesc:
	db   "Umschlag mit Herz-"
	next "Aufdruck. (TRAGEN)@"

EonMailDesc:
	db   "Umschlag mit"
	next "EVOLI. (TRAGEN)@"

MorphMailDesc:
	db   "Umschlag mit"
	next "DITTO. (TRAGEN)@"

BlueSkyMailDesc:
	db   "Himmelblauer"
	next "Umschlag. (TRAGEN)@"

MusicMailDesc:
	db   "Umschlag mit NATU-"
	next "Aufdruck. (TRAGEN)@"

MirageMailDesc:
	db   "Umschlag mit MEW-"
	next "Aufdruck. (TRAGEN)@"

TeruSama29Desc:
	db   "?@"

TeruSama30Desc:
	db   "?@"

TeruSama31Desc:
	db   "?@"

TeruSama32Desc:
	db   "?@"

TeruSama33Desc:
	db   "?@"

TeruSama34Desc:
	db   "?@"

TeruSama35Desc:
	db   "?@"

TeruSama36Desc:
	db   "?@"

TeruSama37Desc:
	db   "?@"
